...
(function(root, factory) {
		...
		return {
			...
			render: function() {
				var _upFlag = '',
					...
					_imgStyle = '', //待定
					_titleStyle = 'font-size: 34px;line-height: 30px;', //待定
					_tipStyle = 'font-size: 24px;line-height: 24px;text-align: right;', //待定
				...
				var _containerHTML = '<div style="text-align: center;background-color: ' + _bgcolor +
					';box-shadow: 1px 1px 4px #888888;padding: 8px 8px 4px;border-radius: 4px;' + _radius + '">',
					_titleHTML = this.isTitleVisibility ?
					'<p class="tt" style="margin:0; text-shadow: 1px 1px rgba(0,0,0,0.1);font-weight: 700;text-align: left;margin-bottom: 4px;' +
					_titleStyle + '">' +
					this.title + '<span id="qrcodeClose" style="float: right;padding: 14px;color: #999;">&times;</span></p>' : '',
					...
					_imgHTML = '<div class="qrcode" style="display: inline-block;width:' + this.settings.qrcodeH + '; height:' + this.settings.qrcodeH +
					'; overflow:hidden;"></div>',
					_tipHTML = this.isTipVisibility ? '<p style="' + _tipStyle + '">' +
					this.tip + '</p>' :
					'',
					_upArrowHTML = '<div style="' + _upFlag +
					'position: relative;height: 0;width: 0;border-style: solid;border-width: 12px;border-color: transparent;border-bottom-color: ' + _bgcolor +
					';border-top: none;"></div>',
					_downArrowHTML = '</div><div style="' + _downFlag +
					'position: relative;height: 0;width: 0;border-style: solid;border-width: 12px;border-color: transparent;border-top-color: ' + _bgcolor +
				...
				this.wxbox.style.cssText = 'position:absolute; left: -100%;width: 100%;height: 100%;z-index: 9999;';
			...
			setLocation: function(flag) {
				...
				_boxStyle = _boxStyle + 'left: 50%;transform: translateX(-50%);';
				...
				this.wxbox.style.width = '98%';
				this.wxbox.style.top = '20%';
				flag && (this.hide());
			},
			bindEvent: function() {
				var _me = this;
				...
				return function() {
					if (!me.qrcode) {
						me.qrcode = new QRCode(Util.getElementByclassN('.qrcode', me.wxbox)[0], {
							text: me.URL,
							width: parseInt(me.settings.qrcodeH),
							height: parseInt(me.settings.qrcodeH),
							colorDark: me.settings.qrcodeFgc,
							colorLight: me.settings.qrcodeBgc,
							addonStyles: me.settings.addonStyles
						});
					}
				}
			},
	...
	function iShare(options) {
		var defaults = {
			...
			image: Util.getimg(),
			sites: ['iShare_weibo', 'iShare_qq', 'iShare_wechat', 'iShare_tencent', 'iShare_douban', 'iShare_qzone', 'iShare_renren', 'iShare_youdaonote',
