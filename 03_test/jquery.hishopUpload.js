    ...
    function uploadfile(target, opts) {
        ...
        if (imgupfile.val() == "") {
            ...
            if (!checkImgType(imgupfile.val(), opts.imgTypes)) {
                if (opts.imgTypes == "") {
                    $(".Prompt").text("上传格式为gif|jpeg|jpg|png|bmp").show(300).delay(3000).hide(300);
                } else {
        ...
        form.ajaxSubmit({
            success: function (data) {
                var imgserverUrl = "";
                var hidImageServerUrl = $("#hidImageServerUrl");
                if (hidImageServerUrl != null || hidImageServerUrl != undefined) {
                    imgserverUrl = $("#hidImageServerUrl").val();
                }

                if ($(data).html().toString().length > $(data).html().toString().replace("error:")) {
                    ...
                    for (var i = 0, j = 0; i < opts.imagesCount; i++) {
                        ...
                        if (started && files[j]) {
                            ...
                            $(hiddenImgSrces[i]).val(files[j++].replace(imgserverUrl, ""));
                            $(uploadBtns[i]).hide();
                            $(imageBoxes[i]).show();
                        }
                    }
    ...
    var renderView = {
        render: function (opts, container) {
            ...
            for (var i = 0; i < opts.imagesCount; i++) {
                ...
                var imgHideUrl = opts.displayImgSrc[i] ? opts.displayImgSrc[i].replace(imgserverUrl, "") : '';
                html += ' <div class="upload-img-box" imageBox ' + display + '>\
                                 ...
                                 <span class="glyphicon remove-img animated swing" style="display:none">删除图片</span>\
                                ...
                                <input class="file uploadFilebtn" style="position:absolute;left:0;top:0;width:100%;height:100%;z-index:1;opacity:0;" type="file" multiple name="_file"   id="imgUploader_' + id + '_' + i + '"/>\
