...
;(function (global, factory) {
    ...
    var utils = {
        ...
        getFormPayload: function (obj, files) {
            ...
            for (var i in obj) {
                if (obj.hasOwnProperty(i)) {
                    ...
                    for (var j in files) {
                        if (files.hasOwnProperty(j) && j === i) {
                            // Content-Type变量参见 http://tool.oschina.net/commons/
                            str += ('; filename="' + files[j] + '"\nContent-Type: application/octet-stream');
                        }
                    }
        ...
        getFormFileName: function (form) {
            ...
            for (var i in elems) {
                ...
                if (nodeAttrs.name) {
                    var v = elem.value;
                    var index = LAST_PATH.exec(v);
                    if (index) {
                        index = index.index;
                        var filename = v.substr(index + 1, v.length);
                        params[nodeAttrs.name.nodeValue] = filename;
                    }
                }
            }
    ...
    XHR.prototype = {
        ...
        _getFormData: function () {
            ...
            var fileNames = utils.getFormFileName(formEle);

            var payload = utils.getFormPayload(formObj, fileNames);
